package com.github.jnoee.xo.demo.action.word.model;
...
public class Software {
  ...
  public Software() {
    ...
    portrait = genPortraitData();
    solutionCompare = genSolutionCompareData();
    feature = genFeatureData();
  }

  private PictureRenderData genPortraitData() {
    try {
      ClassPathResource resource = new ClassPathResource("/META-INF/xo/word/sayi.png");
      return new PictureRenderData(60, 60, ".png",
          BytePictureUtils.toByteArray(resource.getInputStream()));
    } catch (IOException e) {
      throw new SysException("生成图片数据时发生异常。", e);
    }
  }
  ...
  private NumbericRenderData genFeatureData() {
    List<TextRenderData> datas = new ArrayList<>();
